...
def prepare_matches_df(matches):
    ...
    df = df.groupby('group_key', group_keys=False).apply(assign_match_number_within_group)
...
def team_recent_form(df, team_code, max_matchdays=10):
    ...
    group_order = (
        subset.groupby('group_key', as_index=False)
        .agg(last_ts=('sort_timestamp', 'max'), group_label=('group_label', 'first'))
        .sort_values('last_ts', ascending=False, kind='stable')
    )
    keep_keys = group_order.head(max_matchdays)['group_label'].tolist()
    subset = subset[subset['group_label'].isin(keep_keys)].copy()
...
if not df.empty:
    ...
    if not blocks_df.empty:
        ...
        st.bar_chart(
            chart_blocks.set_index('label_chart')[['GG']],
            height=360,
    ...
    if not blocks_df.empty:
        ...
        chart_blocks['label_chart'] = chart_blocks['group_label']
        st.line_chart(
            chart_blocks.set_index('label_chart')[['% sul totale']],
            height=360,
            use_container_width=True
        )
    else:
