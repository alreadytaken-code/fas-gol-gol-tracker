...
st.caption('VERSIONE CODICE: 2026-06-09 22:28 - clean complete')
st.caption(
    'Storico Sisal, forecast blocchi da 6, trend leggibile, ranking manuale GG e centro decisionale operativo.'
)
...
def fetch_matches():
    ...
    response = requests.get(
        ...
        headers={'User-Agent': 'Mozilla/5.0', 'Accept': 'application/json', 'Referer': 'https://www.sisal.it/'}
    )
...
def empty_prepared_df():
    return pd.DataFrame(columns=[
        'match_id', 'api_day', 'timestamp', 'timestamp_str', 'orario', 'giornata', 'codice_palinsesto',
        'codice_avvenimento', 'descrizione_avventimento', 'home_team', 'away_team', 'esito',
...
def prepare_matches_df(matches):
    ...
    required = ['match_id', 'api_day', 'timestamp', 'timestamp_str', 'orario', 'giornata', 'codice_palinsesto',
                'codice_avvenimento', 'descrizione_avventimento', 'home_team', 'away_team', 'esito']
    for col in required:
...
def build_all_gg_stats(df):
    blocks = build_blocks(df)
    if blocks.empty:
        return {'total_all_gg_blocks': 0, 'latest_streak': 0, 'blocks_table': pd.DataFrame(columns=['group_label', 'GG', 'partite', 'all_gg_6su6'])}
    blocks = blocks.copy()
...
def build_probabilities(df):
    ...
    return {'p_ge4': p_ge4, 'p_ge5': p_ge5, 'p_eq6': p_eq6, 'alert_level': alert_level, 'alert_message': msg}


def build_trend_visual_df(df):
    blocks = build_blocks(df)
    if blocks.empty:
        return pd.DataFrame(columns=['group_label', 'label_chart', 'GG', '% sul totale', 'gg_ma_3', 'gg_ma_5', 'pct_ma_3', 'pct_ma_5', 'cycle_id', 'giornata', 'orario_inizio'])
...
def build_trend_status(df):
    ...
    if chart_df.empty:
        return {'label': 'Nessun dato', 'delta_short_vs_long': 0.0, 'momentum': 0, 'stress': 0, 'last_gg': 0, 'last_pct': 0.0, 'last_vs_ma5': 0.0}
    ...
    return {'label': label, 'delta_short_vs_long': delta, 'momentum': momentum, 'stress': stress, 'last_gg': last_gg, 'last_pct': last_pct, 'last_vs_ma5': last_vs_ma5}
...
def build_model_diagnostics(df):
    ...
    if blocks.empty or backtest['table'].empty:
        return {'hit_rate': 0.0, 'error_proxy': 0.0, 'hot_blocks_rate': 0.0, 'cold_blocks_rate': 0.0}
    ...
    bt['pred_round'] = bt['predicted_GG'].round()
    bt['hit'] = (bt['pred_round'] == bt['actual_GG']).astype(int)
    hit_rate = round(float(bt['hit'].mean() * 100), 2) if not bt.empty else 0.0
    error_proxy = round(float((backtest['mae_10'] + backtest['mae_20']) / 2), 2)
    return {'hit_rate': hit_rate, 'error_proxy': error_proxy, 'hot_blocks_rate': hot_blocks_rate, 'cold_blocks_rate': cold_blocks_rate}
...
def get_team_trend_5_10(df, team_code):
    ...
    return {'team': team_code, 'rate_5': rate_5, 'rate_10': rate_10, 'trend_score': trend_score, 'matches_5': matches_5, 'matches_10': matches_10}
...
def build_match_scores(input_df, history_df):
    if input_df.empty:
        return pd.DataFrame(columns=['match', 'home_team', 'away_team', 'quota_gg', 'prob_mercato', 'home_rate_5', 'home_rate_10', 'away_rate_5', 'away_rate_10', 'team_trend_avg', 'global_trend', 'bonus_classifica', 'score_finale', 'edge', 'ev_pct', 'stake_units', 'confidence_band'])
    ...
    for _, r in input_df.iterrows():
        ...
        ev_pct = compute_ev_percent(score_finale, r['quota_gg'])
        stake_units = suggested_stake_units(edge, score_finale)
        rows.append({
            ...
            'edge': edge, 'ev_pct': ev_pct, 'stake_units': stake_units, 'confidence_band': confidence_band(score_finale),
            'home_matches_5': home_trend['matches_5'], 'home_matches_10': home_trend['matches_10'],
...
def build_manual_summary(pred_df, expected_gg_total, gg_slots):
    if pred_df.empty:
        return {'matches_count': 0, 'expected_gg_total': 0.0, 'expected_gg_rounded': 0, 'predicted_gg_count': 0, 'predicted_ng_count': 0}
    return {
...
def build_score_bands_table(pred_df):
    ...
    out = work.groupby('fascia', observed=False).agg(
        partite=('match', 'count'),
        GG_predetti=('prediction', lambda x: int((x == 'GG').sum())),
        stake_medio=('stake_units', 'mean')
    ).reset_index()
...
if not df.empty:
    ...
    s3.metric('Ultima giornata rilevata', int(df['giornata'].max()) if 'giornata' in df.columns else 0)
    ...
    col3.metric('% GG ultimo blocco', f"{trend['latest_block_pct']}%")

    st.subheader('Previsione prossimi blocchi')
    forecast = build_forecast(df)
    fc1, fc2, fc3 = st.columns(3)
    fc1.metric('GG attesi prossimo blocco', forecast['next_block_expected'])
    fc2.metric('Stima arrotondata', forecast['next_block_rounded'])
    ...
    st.caption(f"Ultimo blocco: {trend_status['last_gg']} GG ({trend_status['last_pct']:.1f}%) | Scostamento vs MA5: {trend_status['last_vs_ma5']}")
    ...
    if not trend_chart_df.empty:
        ...
        st.caption('Asse Y fisso 0-6. Etichette: Giornata + orario. Colori: rosso <=2, giallo =3, verde >3.')
    else:
    ...
    with cfb:
        ...
        st.caption("Per ogni blocco vedi giornata, orario e quanti GG sono usciti.")
    ...
    with st.expander('Partite giorno per giorno', expanded=False):
        ...
        for label in ordered_labels:
            ...
            with st.expander(f'Giornata {giornata_value} · Partite {len(blocco_g)} · GG {gg_count} · NG {ng_count}', expanded=False):
                st.dataframe(blocco_g[['match_nel_blocco', 'orario', 'giornata', 'codice_avvenimento', 'descrizione_avventimento', 'esito']].rename(columns={'match_nel_blocco': 'n_match'}), use_container_width=True, hide_index=True)
...
if not raw_text.strip():
    ...
    g5.metric('Stake totale suggerito', round(float(pred_df['stake_units'].sum()), 2) if not pred_df.empty else 0.0)
    ...
    st.markdown('### Sintesi strategica')
    s1, s2, s3 = st.columns(3)
    s1.metric('Giocate alta confidenza', int((pred_df['confidence_band'] == 'Alta').sum()) if not pred_df.empty else 0)
    s2.metric('Giocate media confidenza', int((pred_df['confidence_band'] == 'Media').sum()) if not pred_df.empty else 0)
    s3.metric('Partite +EV', int((pred_df['edge'] > 0).sum()) if not pred_df.empty else 0)
    ...
    for col in ['prob_mercato', 'home_rate_5', 'home_rate_10', 'away_rate_5', 'away_rate_10', 'team_trend_avg', 'global_trend', 'bonus_classifica', 'score_finale', 'edge']:
    ...
    display_df.columns = [
        ...
        'Trend medio match %', 'Trend globale %', 'Bonus classifica %', 'Score finale %', 'Edge %', 'EV %', 'Stake', 'Confidenza',
        'Casa match 5g', 'Casa match 10g', 'Trasferta match 5g', 'Trasferta match 10g', 'Previsione'
    ]
